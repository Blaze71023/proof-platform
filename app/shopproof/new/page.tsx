...
export default function JobDetailPage() {
  ...
  const vehicleTitle = [job.vehicleYear, job.vehicleMake, job.vehicleModel]
    .filter(Boolean)
    .join(" ") || "Vehicle Record";
      ...
      <div style={shellStyle}>
        ...
        <div style={bodyGridStyle}>
          <main style={mainColumnStyle}>
            ...
            <Panel title="Intake Snapshot" subtitle="The customer concern and intake context carried from creation.">
          ...
          <aside style={sideColumnStyle}>
            <Panel title="Workflow Actions" subtitle="Keep the record moving without changing the intake source.">
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <div style={helpBoxStyle}>
                  Final Release should read this same normalized record shape so customer,
                  vehicle, concern, mileage, fee, and writer details do not disappear.
                </div>
              </div>
            </Panel>
            ...
            <Panel title="Record Attribution" subtitle="Captured intake authority.">
              <InfoGrid single>
                <InfoItem label="Diagnostic Fee" value={job.diagnosticFee ? `$${job.diagnosticFee}` : ""} />
...
function normalizeJob(job: AnyJob): NormalizedJob {
  ...
  return {
    ...
    customerAddress: firstNonEmpty(job?.customer_address, job?.customerAddress, customer?.address),
    ...
    requestedWork: firstNonEmpty(job?.requested_work, job?.requestedWork, parseSnapshotValue(notes, "Requested Work")),
    notes,
...
function statusPillStyle(status: string): CSSProperties {
  ...
  return {
    ...
    border: `1px solid ${isComplete ? THEME.emeraldLine : isProgress ? THEME.blueLine : THEME.amberLine}`,
    whiteSpace: "nowrap",
  };
}
