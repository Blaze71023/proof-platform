...
export default function ShopProofNewPage() {
  ...
  const handleCreateJob = async () => {
    ...
    try {
      ...
      if (!customerId && normalizedPhoneDigits.length === 10) {
        const phoneFormats = Array.from(
          new Set([
            ...
            formatPhone(normalizedPhoneDigits),
            normalizedPhone,
          ])
        );
        ...
        customerId = (phoneMatches as Array<CustomerRow> | null)?.[0]?.id ?? null;
      }
      ...
      fallbackToLocal(detailedMessage);
    } finally {
      setIsCreating(false);
    }
  };
